...
(function factory(root) {
  ...
  */
   ...
   * - base64.encodeArrayBuffer(aArrayBuffer) : sBase64
   ...
   *
   * If this script is running on nodejs you can use:
   *
   * - base64.encodeBuffer(aArrayBuffer) : sBase64
   * - base64.decodeAsBuffer(sBase64, [nBlocksSize]) : aArrayBuffer
   */
  ...
  function base64DecodeAsUint8Array(sBase64, nBlocksSize, AltCons) {
    /*jslint regexp: true, bitwise: true */
    var nMod3, nMod4, nUint24 = 0, nOutIdx = 0, nInIdx,
      sB64Enc = sBase64.replace(/[^A-Za-z0-9\+\/]/g, ""),
      nInLen = sB64Enc.length,
      nOutLen = nBlocksSize ? Math.ceil((nInLen * 3 + 1 >> 2) / nBlocksSize) * nBlocksSize : nInLen * 3 + 1 >> 2,
      taBytes = AltCons && new AltCons(nOutLen) || new Uint8Array(nOutLen);
  ...
  function base64EncodeBinaryString(sBytes) {
    var i, l = sBytes.length, ua = new Uint8Array(l);
    for (i = 0; i < l; i += 1) {
      ua[i] = sBytes.charCodeAt(i);
    }
  ...
  function base64DecodeAsBuffer(sBase64, nBlocksSize) {
    return base64DecodeAsUint8Array(sBase64, nBlocksSize, Buffer);
  }

  base64.decodeAsBuffer = base64DecodeAsBuffer;
  base64.encodeBuffer = base64EncodeUint8Array;
